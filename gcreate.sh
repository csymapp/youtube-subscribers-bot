#!/bin/bash
#### image captcha encountered after a number of tries
set +H
# username=$1
...
###
  ...
  #### check the contents of the page again
...
#### check if "not a robot" is in the page opened

if grep -q "not a robot" "$output_file"; then
  echo "not a robot' was found in the file."
    else
    echo "$email" >> ./createdAccounts # assume link has been used
    continue
fi
break
  ## fill in form
